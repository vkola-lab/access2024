...
def add_ventricle_info(parcellation_df, ventricle_df, ventricles):
    return parcellation_df.merge(
        ventricle_df[["RID"] + ventricles], on="RID", validate="one_to_one"
    )
...
def CNR(tensor):
    ...
    for slice_idx in [80]:
        ...
        return np.abs(np.mean(roi1) - np.mean(roi2)) / np.sqrt(
            np.square(np.std(roi1)) + np.square(np.std(roi2))
        )
...
def SSIM(tensor1, tensor2, zoom=False):
    ...
    for slice_idx in [60]:
        if zoom:
            ...
            side_b = slice_idx + 30
            img1, img2 = (
                tensor1[side_a:side_b, side_a:side_b, 60],
                tensor2[side_a:side_b, side_a:side_b, 60],
            )
        else:
...
def immse(tensor1, tensor2, zoom, eng):
    ...
    for slice_idx in [50, 80, 110]:
        if zoom:
            ...
            side_b = slice_idx + 60
            img1, img2 = (
                tensor1[side_a:side_b, side_a:side_b, 105],
                tensor2[side_a:side_b, side_a:side_b, 105],
            )
        else:
            img1, img2 = tensor1[slice_idx, :, :], tensor2[slice_idx, :, :]
...
def retrieve_kfold_partition(
    idxs, stage, folds=5, exp_idx=1, shuffle=True, random_state=120
):
...
def read_csv_demog_ed_apoe(filename, skip_ids: list = None):
    with open(filename, "r") as f:
        reader = csv.DictReader(f)
        fileIDs, time_obs, hit, age, mmse, sex, educ, apoe = (
            [],
            [],
            [],
            [],
...
if __name__ == "__main__":
    ...
    csvname = "./csvs/merged_dataframe_cox_noqc_pruned_final.csv"
    csvname = os.path.expanduser(csvname)
    read_csv_demog_ed_apoe(csvname)
